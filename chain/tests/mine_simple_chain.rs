...
fn mine_reorg() {
	...
	const DIR_NAME: &str = ".kepler_reorg";
	clean_output_dir(DIR_NAME);
...
#[test]
fn mine_forks() {
	clean_output_dir(".kepler2");
	global::set_mining_mode(ChainTypes::AutomatedTesting);
	{
		let chain = init_chain(".kepler2", pow::mine_genesis_block().unwrap());
		let kc = ExtKeychain::from_random_seed(false).unwrap();
...
#[test]
fn mine_losing_fork() {
	clean_output_dir(".kepler3");
	global::set_mining_mode(ChainTypes::AutomatedTesting);
	let kc = ExtKeychain::from_random_seed(false).unwrap();
	{
		let chain = init_chain(".kepler3", pow::mine_genesis_block().unwrap());
...
fn longer_fork() {
	...
	let genesis = pow::mine_genesis_block().unwrap();
	{
		let chain = init_chain(".kepler4", genesis.clone());
...
fn spend_rewind_spend() {
	global::set_mining_mode(ChainTypes::AutomatedTesting);
	util::init_test_logger();
	clean_output_dir(".kepler_spend_rewind_spend");

	{
		let chain = init_chain(
			".kepler_spend_rewind_spend",
...
fn spend_in_fork_and_compact() {
	clean_output_dir(".kepler6");
	global::set_mining_mode(ChainTypes::AutomatedTesting);
	util::init_test_logger();
	{
		let chain = init_chain(".kepler6", pow::mine_genesis_block().unwrap());
		let prev = chain.head_header().unwrap();
		let kc = ExtKeychain::from_random_seed(false).unwrap();
		let pb = ProofBuilder::new(&kc);
...
fn output_header_mappings() {
	...
	// Cleanup chain directory
	clean_output_dir(".kepler_header_for_output");
}
