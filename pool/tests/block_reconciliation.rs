...
fn test_transaction_pool_block_reconciliation() {
	...
	let db_root = ".kepler_block_reconciliation".to_string();
	clean_output_dir(db_root.clone());
	{
		...
		let header = {
			...
			let reward = libtx::reward::output(&keychain, &key_id, 0, height, false).unwrap();
		...
		let block = {
			...
			let reward =
				libtx::reward::output(&keychain, &key_id, fees, header.height, false).unwrap();
			let mut block =
